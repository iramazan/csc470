...
var shader_program;
var vertices = [
     0.33,  0.33, 0.1, 1, // top right
     0.33, -0.33, -0.5, 1, // bottom right
    -0.33, -0.33, 0.2, 1, // bottom left
...
function texture_init()
{
    ...
    var image = new Image();
    image.src = "brickwork-texture.jpg";
    ...
    // load normal map too
    var nm_texture = gl.createTexture();
    var nm_image = new Image();
    nm_image.src = "brickwork_normal-map.jpg";
    nm_image.addEventListener('load', function() {
        gl.bindTexture(gl.TEXTURE_2d, nm_texture);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, nm_image);
        // different requirements if image is power of 2 or not
        if (image.width & (image.width - 1) == 0 &&
            image.height & (image.height - 1) == 0) {
            gl.generateMipmap(gl.TEXTURE_2D);
    ...
    gl.bindTexture(gl.TEXTURE_2D, nm_texture);
    var normal_map_location = gl.getUniformLocation(shader_program, "normal_map");
